...
void DrawSCRSShipyard(S_ANIMATION **animHead, INTERFACEACTION **actionHead){
	...
	cur.h = 0;
	if(CastleNearBoat()){
	    FillSPRITE(&sprite, "TWNSBOAT.ICN", 1);
	    header = GetICNHeader(&sprite);
    ...
    // спрайт
    if(CastleNearBoat()) FillSPRITE(&sprite, "TWNSBOAT.ICN", 0); else FillSPRITE(&sprite, "TWNSDOCK.ICN", 0);
    header = GetICNHeader(&sprite);
    cur.x = cx + header->offsetX;
    cur.y = cy + header->offsetY;
    cur.w = header->surface->w;
...
void DrawSCRSDwelling2(S_ANIMATION **animHead, INTERFACEACTION **actionHead){
    ...
    if(castle->building & DWELLING_UPGRADE2){ icnname = "TWNSUP_1.ICN"; }else{ icnname = "TWNSDW_1.ICN"; }
    INTERFACEACTION action;
...
void DrawSCRSDwelling3(S_ANIMATION **animHead, INTERFACEACTION **actionHead){
    ...
    const S_CASTLE *castle = GetCurrentCastle();
    const char *icnname = NULL;
    if(castle->building & DWELLING_UPGRADE3){ icnname = "TWNSUP_2.ICN"; }else{ icnname = "TWNSDW_2.ICN"; }
    INTERFACEACTION action;
...
void DrawSCRSDwelling4(S_ANIMATION **animHead, INTERFACEACTION **actionHead){
    ...
    const S_CASTLE *castle = GetCurrentCastle();
    const char *icnname = NULL;
    if(castle->building & DWELLING_UPGRADE4){ icnname = "TWNSUP_3.ICN"; }else{ icnname = "TWNSDW_3.ICN"; }
    INTERFACEACTION action;
