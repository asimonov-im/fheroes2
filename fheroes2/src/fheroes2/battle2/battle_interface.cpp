...
void Battle2::Interface::HumanTurn(const Stats & b, Actions & a)
{
	...
	// animation troops
	if(IdleTroopsAnimation(ticket)) humanturn_redraw = true;
...
void Battle2::Interface::RedrawActionAttackPart1(Stats & attacker, Stats & defender, const std::vector<TargetInfo> & targets)
{
    ...
    // check archers
    const bool archer = attacker.isArchers() && 1 < Board::GetDistance(attacker.GetPosition(), defender.GetPosition());
...
bool Battle2::Interface::IdleTroopsAnimation(u32 ticket)
{
    bool res = false;
