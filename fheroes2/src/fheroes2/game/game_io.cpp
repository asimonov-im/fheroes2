...
bool Game::IO::LoadBIN(QueueMessage & msg)
{
    ...
    {
	msg.Pop(byte8);
	const Heroes::heroes_t id = Heroes::ConvertID(byte8);
	msg.Pop(byte8);

	Heroes* hero = new Heroes(id, Race::Get(byte8));
    ...
    {
	Point center;
	msg.Pop(byte16); center.x = byte16;
	msg.Pop(byte16); center.y = byte16;
	msg.Pop(byte8);

	Castle* castle = new Castle(center.x, center.y, Race::Get(byte8));
	...
	// funds
	msg.Pop(size); kingdom->resource.wood = size;
	msg.Pop(size); kingdom->resource.mercury = size;
	msg.Pop(size); kingdom->resource.ore = size;
	msg.Pop(size); kingdom->resource.sulfur = size;
	msg.Pop(size); kingdom->resource.crystal = size;
	msg.Pop(size); kingdom->resource.gems = size;
	msg.Pop(size); kingdom->resource.gold = size;
